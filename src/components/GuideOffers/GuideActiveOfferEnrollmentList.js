            ...
            }
            {
                dropdown ?
                    <Row className={"mb-5 d-flex justify-content-end justify-content-xl-start"}
                        ...
                        <Col xl={3} md={5} xs={10}
                             className={"d-flex flex-column p-xl-5 justify-content-start mt-5 mt-xl-0"}>
                                            ...
                                            <Col xl={6} className={"d-flex flex-column p-5 justify-content-start"}>
                                                {
                                                    enrollmentDetails.message_threads.length > 0 ?
                                                        <DropdownButton
                                                            title={`Liczba konwersacji z tym klientem: ${enrollmentDetails.message_threads.length} `}
                                                            className={"mb-3"} variant={"warning"}>
                                                            {
                                                                enrollmentDetails.message_threads.map((thread, index) => {
                                                                    return (
                                                                        <DropdownItem key={index} onClick={() => {
                                                                            history.push(`/account/guide/active-offer/${tourID}/messages`)
                                                                        }}>
